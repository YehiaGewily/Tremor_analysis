...
class DataVisualizer:
    ...
    def visualize_single_day(self, day_label: str, smoothing_window=11, show_plot=False):
        """Create visualization for a single day/label."""
        ...
        viable_cols = [col for col in numerical_cols if col != index_col and col in original_df.columns and col in predicted_df.columns]
        ...
        for i, col in enumerate(numerical_cols):
            # Skip index column if it was included in numerical columns
            if col == index_col:
                continue
            ...
            if col in original_df.columns:
                ...
                # Plot original raw data as scatter and smoothed as line
                ax.scatter(original_df[index_col], orig_data, 
                           alpha=0.3, s=15, color='blue', label='Original (raw)')
                ax.plot(original_df[index_col], orig_smoothed, 
                        linewidth=2, color='darkblue', label='Original (smoothed)')
            ...
            if col in predicted_df.columns:
                ...
                # Plot predicted raw data as scatter and smoothed as line
                ax.scatter(predicted_df[index_col], pred_data, 
                           alpha=0.3, s=15, color='red', label='Predicted (raw)')
                ax.plot(predicted_df[index_col], pred_smoothed, 
                        linewidth=2, color='darkred', label='Predicted (smoothed)')
            
            # Add error between original and predicted if both exist
            if col in original_df.columns and col in predicted_df.columns:
                # Calculate error
                error = np.abs(original_df[col] - predicted_df[col])
                
                # Now fill_between will work because datasets are aligned
                ax.fill_between(
        ...
        # Add a summary plot at the end
        common_cols = [col for col in numerical_cols if col in original_df.columns and col in predicted_df.columns and col != index_col]
        ...
        if common_cols and len(common_cols) > 0 and plot_idx <= n_rows * 2:
            try:
                ...
                if rmse_values:
                    ...
                    bars = ax_summary.bar(common_cols, rmse_values)
                    ax_summary.set_title('RMSE Summary')
                    ax_summary.set_xlabel('Feature')
                    ax_summary.set_ylabel('RMSE')
    ...
    def create_heatmap_comparison(self, show_plot=False):
        ...
        for day_label in self.file_pairs.keys():
            ...
            numerical_cols = [
                ...
                if col in predicted_df.columns and col != index_col
            ]
